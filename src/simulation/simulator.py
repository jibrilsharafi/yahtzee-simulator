...
class Simulator:
    ...
    def run_simulation(self, strategy: BaseStrategy, game_state: Dict[str, Any], dice_roll: List[int]) -> Tuple[Set[int], str]:
        """
        ...
        """
        dice_to_keep = strategy.select_dice_to_keep(dice_roll, game_state['current_player'].scorecard)
        category = strategy.select_category(dice_roll, game_state['current_player'].scorecard)
        
        self.results.append({
            'dice_roll': dice_roll,
            'kept_dice': dice_to_keep,
            'category': category,
            'potential_score': game_state['current_player'].scorecard.calculate_score(category, dice_roll)
        })
        
        return dice_to_keep, category

    def run_game_simulation(self, num_games: int, strategies: Dict[str, BaseStrategy]) -> Dict[str, int]:
        """
